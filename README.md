...
t=$(date +%d\\/%b\\/%G:$(date --date '-60 min' +%H))      # переменная периода анализа, актуальное время минус 60 мин
...
then                                                # активная часть скрипта, выставляется трап
  ...
  sleep 30                                          # ставится условие, если запись не удалась вывести сообщение 
  rm -f "$lockfile"                                 # о том что скрипт запущен, если удалась, то запустить ml(), 
